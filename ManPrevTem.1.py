...
#========================================= -:fim dos Frames:- =========================================
        ...
        self.label_cadas = Label(self.abas_pg0, text = 'CADASTRO', font = ('Ariel', '25'), bg = self.cor_do_fundo)# criei o label=escrita
        self.label_cadas.place(relx=0.12, rely=0.03) # off-set do texto (posicionamento)       
      
        self.label_placa = Label(self.abas_pg0, text = 'Placa', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_placa.place(relx=0.02, rely=0.15) # off-set do texto (posicionamento)
        self.entrada_placa = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_placa.place(relx=0.02,rely=0.20)

        self.label_cliente = Label(self.abas_pg0, text = 'Cliente', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_cliente.place(relx=0.02, rely=0.25) # off-set do texto (posicionamento)
        self.entrada_placa = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_placa.place(relx=0.02, rely=0.30)


        self.label_ender = Label(self.abas_pg0, text = u'Endereço', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_ender.place(relx=0.02, rely=0.35) # off-set do texto (posicionamento)

#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  Corrigir função  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

        # Corrigir entrada de texto de forma que a linha de digitação seja expansiva.
        self.entrada_ender = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_ender.place(relx=0.02, rely=0.40)

        self.label_cid = Label(self.abas_pg0, text = u'Cidade', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_cid.place(relx=0.02, rely=0.45) # off-set do texto (posicionamento)
        self.entrada_cid = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_cid.place(relx=0.02, rely=0.50)

        self.label_cep = Label(self.abas_pg0, text = u'CEP', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_cep.place(relx=0.02, rely=0.55) # off-set do texto (posicionamento)
        self.entrada_cep = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_cep.place(relx=0.02, rely=0.60)

        self.label_cpf = Label(self.abas_pg0, text = u'CPF/CNPJ', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_cpf.place(relx=0.02, rely=0.65) # off-set do texto (posicionamento)
        self.entrada_cpf = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_cpf.place(relx=0.02, rely=0.70)

        self.label_tel = Label(self.abas_pg0, text = u'Telefone', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_tel.place(relx=0.02, rely=0.75) # off-set do texto (posicionamento)
        self.entrada_tel = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_tel.place(relx=0.02, rely=0.80)


        self.label_email = Label(self.abas_pg0, text = u'E-mail', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_email.place(relx=0.02, rely=0.85) # off-set do texto (posicionamento)
        self.entrada_email = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_email.place(relx=0.02, rely=0.90)

#====================================== -:Aba Check-List:- ================================== 

        self.label_age = Label(self.abas_pg1, text = 'Agenda entrada',  bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_age.place(relx=0.02, rely=0.05 ) # off-set do texto (posicionamento)
        ...
        self.label_age = Label(self.abas_pg1, text = u'Previsão',  bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_age.place(relx=0.23, rely=0.05 )
        self.entrada_age = Entry(self.abas_pg1, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_age.place(relx=0.22,rely=0.10,relheight=0.042,relwidth=0.1)

        self.label_age = Label(self.abas_pg1, text = u'saida',  bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_age.place(relx=0.38, rely=0.05 )
        self.entrada_age = Entry(self.abas_pg1, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_age.place(relx=0.37,rely=0.10,relheight=0.042,relwidth=0.1)

        self.label_vmar = Label(self.abas_pg1, text = 'Veiculo Marca', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_vmar.place(relx=0.04, rely=0.15) # off-set do texto (posicionamento)
        ...
        self.label_model = Label(self.abas_pg1, text = 'Modelo', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        ...
        self.label_ano = Label(self.abas_pg1, text = 'Ano', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        ...
        self.label_combust = Label(self.abas_pg1, text = u'Nivel de Combustivel', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_combust.place(relx=0.02, rely=0.275) # off-set do texto (posicionamento)
        ...
        self.label_klm = Label(self.abas_pg1, text = u'Kilometragem ', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_klm.place(relx=0.02, rely=0.45) # off-set do texto (posicionamento)
        ...
        self.label_kfer = Label(self.abas_pg1, text = u'Kit de ferramentas', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_kfer.place(relx=0.02, rely=0.6) # off-set do texto (posicionamento)

        self.label_mac = Label(self.abas_pg1, text = u'Macaco', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_mac.place(relx=0.02, rely=0.67) # off-set do texto (posicionamento)

        self.label_ext = Label(self.abas_pg1, text = u'Extintor', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_ext.place(relx=0.02, rely=0.74) # off-set do texto (posicionamento)

        self.label_estep = Label(self.abas_pg1, text = u'Estepe', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_estep.place(relx=0.02, rely=0.81) # off-set do texto (posicionamento)
        
        self.label_sint = Label(self.abas_pg1, text = u'Sintomas apresentados:', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_sint.place(relx=0.62, rely=0.06) # off-set do texto (posicionamento)
        ...
        self.label_diag = Label(self.abas_pg1, text = u'Diagnóstico:', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_diag.place(relx=0.68, rely=0.32) # off-set do texto (posicionamento)
        ...
        self.label_diag = Label(self.abas_pg1, text = u'Serviço executado:', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_diag.place(relx=0.68, rely=0.6) # off-set do texto (posicionamento)
        self.entrada_diag = Text(self.abas_pg1, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        ...
        self.label_pesq = Label(self.abas_pg0, text = 'PESQUISAR', bg = self.cor_do_fundo, font = ('Ariel', '25'))# criei o label=escrita
        self.label_pesq.place(relx=0.65, rely=0.03) # off-set do texto (posicionamento)


        self.label_cons = Label(self.abas_pg0, text = u'consultar', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_cons.place(relx=0.52, rely=0.18) # off-set do texto (posicionamento)
        self.entrada_cons = Entry(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) 
        self.entrada_cons.place(relx=0.52, rely=0.23)


        self.label_com = Label(self.abas_pg0, text = 'Complemento', bg = self.cor_do_fundo, font = ('Ariel', '15'))# criei o label=escrita
        self.label_com.place(relx=0.52, rely=0.35) # off-set do texto (posicionamento)

        self.entrada_com = Text(self.abas_pg0, bg = self.cor_do_entrada, font = ('Ariel', '15')) # itém modificado
        # era Entry mudou para Text que na verdade é uma cx de texto
        #self.entrada_com.place(relx=0.52, rely=0.40)
        self.entrada_com.place(relx=0.52,rely=0.40,relheight=0.52,relwidth=0.45) 



        #self.check_com = Text(self.abas_pg1, bg = self.cor_do_entrada, font = ('Ariel', '15')) # itém modificado
        #self.check_com.place(relx=0.52,rely=0.10,relheight=0.87,relwidth=0.45)
